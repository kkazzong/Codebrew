	...
	<script>
		...
		$(function(){
			...
			$("#sendButton").on('click', function(event){
				...
				var roomName = $('#roomNameInput').val(); //SR1.jsp에서 추가
				...
				var output = {
						command : chatType,
						roomId : roomId,
						roomName : roomName, //SR1.jsp에서 추가
						sender : sender,
						recipient : recipient, 
						senNick : senNick,
						recNick : recNick,
			...
			$("#dataInput").on('keydown',function(event){
				if(event.keyCode ==13){
					...
					var senNick = $('#senNick').val(); //////////senNick추가////////////////////
					var recNick = $('#recNick').val();
					var senImg = $('#senImg').val(); //SR1.jsp에서 추가
					var data = $('#dataInput').val();
					var time = $('#dataInputTime').val();
		...
		function addToGroupDiscussion(writer, message){
			...
			if(writer == 'other'){
				...
				//recipient = '${recipient.nickname}';
				///////////////recipient는 서버에서 받은걸로
				img = message.senImg; //SR1.jsp에서 추가
...
<body>
	...
	<div>
		...
		<!-- SR1.jsp에서 추가 -->
		<input type = "hidden" id = "senImg" value="${sender.profileImage}">
