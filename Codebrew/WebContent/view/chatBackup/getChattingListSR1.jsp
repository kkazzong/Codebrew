...
<head>
	...
	<script>
		...
		function getPartyAjax(partyNo, message) {
			...
			$.ajax({
				...
				success : function(JSONData) {
					...
					/* message.roomName =  JSONData.partyName; */
					roomName = JSONData.partyName;
					console.log("파티이름!!!!"+JSONData.partyName);
		...
		function connectToServer(){
			...
			socket.on('connect', function(){
				...
				socket.on('findGroupChatList', function(message){
					...
					alert(JSON.stringify(message.length));
					for(var i = 0; i < message.length; i++) {
						var roomId = message[i].roomId;
						var members = message[i].members;
						alert(roomId+", "+members+", "+message[i].roomName);
						//getPartyAjax(roomId);
						//getPartyAjax(roomId, message[i]);
						//var roomName = getPartyAjax(roomId, message[i]);
						//message[i].roomName = roomName; 
						//console.log("파이널메세지!!!!"+getPartyAjax(roomId, message[i]));
						//alert("에이작스에서 얻은 파티네임"+roomName);
						
						addToGroupDiscussion(message[i]); 
						/* addToGroupDiscussion(finalMessage); */
					}
				...
				////////////////////////////findRoomName//////////////////////////////////
				socket.on('findRoomName', function(roomName){
					
					console.log("FROM DB"+JSON.stringify(roomName));
		...
		function addToGroupDiscussion(message, roomName){
